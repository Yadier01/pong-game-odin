...
main :: proc() {
    ...
    for !rb.WindowShouldClose() {
        ...
        if rb.CheckCollisionCircleRec({ auto_cast ballState.circlePos.x, auto_cast ballState.circlePos.y },
        ballState.circleRadius, {
            auto_cast playerRecPos.x, auto_cast playerRecPos.y, auto_cast recWidth, auto_cast recHight
        }){
        ...
        if  rb.CheckCollisionCircleRec({ auto_cast ballState.circlePos.x, auto_cast ballState.circlePos.y }, ballState.circleRadius, {
            auto_cast cpuState.cpuPosition.x, auto_cast cpuState.cpuPosition.y, auto_cast recWidth, auto_cast recHight
        }){
            ballState.ballSpeedX = -ballState.ballSpeedX
            randomVal := rand.float32()
            if randomVal < 0.5 {
                ballState.ballSpeedY = + ballState.ballSpeedY
                fmt.println("random val is less than 0.6", randomVal)
            }
            fmt.println("hit cpu", randomVal)
        }
        ballCollisionWithWall(&ballState, &gameState)

        if ballState.circlePos.y > cpuState.cpuPosition.y && cpuState.cpuPosition.y + recHight < windowHight{
        ...
        //cpu
        rb.DrawRectangle(cpuState.cpuPosition.x, cpuState.cpuPosition.y, recWidth, recHight, rb.RED)
...
centered_text_int :: proc(x: int, y: int, value: int, fontSize: int) {
    ...
    rb.DrawText(text_cstr, auto_cast x - (textWidth / 2), auto_cast y, auto_cast fontSize, rb.WHITE)
}
